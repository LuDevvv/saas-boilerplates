  ...
  return (
    <Card className="p-8 rounded-[32px] border-border-subtle bg-white dark:bg-white/5 shadow-sm">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h3 className="text-sm font-heading text-fg uppercase">Almacenamiento</h3>
      </div>

      <div className="space-y-6">
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <span className="text-2xl font-kpi text-fg">
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-50 dark:border-white/5">
          <div className="space-y-1">
            ...
            <p className="text-sm font-label text-fg">{isLoading ? "-" : fileCount}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[10px] font-heading text-slate-400 uppercase flex items-center gap-1.5">
              <Database size={10} /> Cuota
            </p>
            <p className="text-sm font-label text-fg">{Math.round(percentUsed)}% usado</p>
          </div>
        </div>
      </div>
    </Card>
