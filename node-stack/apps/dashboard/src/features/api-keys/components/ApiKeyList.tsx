...
import type { ApiKey } from "../../workspaces/api/api-keys.api";
...
export const ApiKeyList: FC<ApiKeyListProps> = ({ keys, isLoading, onRevoke }) => {
  ...
  return (
    <div className="space-y-4">
      {keys.map((key) => (
        <Card key={key.id} className="p-6 rounded-[24px] border-slate-100 dark:border-white/5 bg-white dark:bg-white/5 shadow-sm group hover:shadow-md transition-all duration-300">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              <div className="p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 group-hover:scale-110 transition-transform duration-500">
                <Lock className="w-5 h-5 text-slate-400" />
              </div>
              <div className="space-y-1">
                <h4 className="text-base font-heading text-slate-900 dark:text-white">{key.name}</h4>
